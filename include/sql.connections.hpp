...
{
   //dbase
   static std::wstring SQL_DBASE = (L"Driver={Microsoft dBASE Driver (*.dbf)};DriverID=277;"
                   "Dbq=c:\\DatabasePath;");
   //excel
   static std::wstring SQL_EXCEL = (L"Driver={Microsoft Excel Driver (*.xls)};DriverId=790;"
    "Dbq=%1;DefaultDir=%2;");

   //text
   static std::wstring SQL_TEXT = (L"Driver={Microsoft Text Driver (*.txt; *.csv)};"
        "Dbq=%1;Extensions=asc,csv,tab,txt;");
   //create schema.ini where information
   //Format=TabDelimited

   //visual foxpro
   static std::wstring SQL_VISUAL_FOXPRO = (L"Driver={Microsoft Visual Foxpro Driver};UID=;"
    "SourceType=%1;SourceDB=%2;Exclusive=%3");

   //If you are working without a Database container, you must change the SourceType 
   //parameter by DBF as in the following connection std::wstring:

   static std::wstring SQL_VISUAL_FOXPRO_1 = (L"Driver={Microsoft Visual Foxpro Driver};UID=;"
    "SourceType=DBF;SourceDB=%1;Exclusive=%2");

   //ODBC Driver for Access
   //Collapse | Copy Code
   static std::wstring SQL_ACCESS = (L"Driver={Microsoft Access Driver (*.mdb)};"
           "Dbq=%1;Uid=;Pwd=;");

   //If you are using a Workgroup (System Database): you need to inform the SystemDB Path, the User Name and its password. For that, you have two solutions: inform the user and password in the connection std::wstring or in the moment of the Open operation. For example:

    //Collapse | Copy Code
   static std::wstring SQL_ACCESS_V1 = (L"Driver={Microsoft Access Driver (*.mdb)};"
           "Dbq=%1;SystemDB=%2;Uid=%3;Pwd=%4");

//   static std::std::wstring strConnection = _T("Driver={Microsoft Access Driver (*.mdb)};"
...
////
//// Collapse | Copy Code
//   static std::std::wstring strConnection = _T("Driver={SQL Server};Server=MyServerName;"
...
////
//// Collapse | Copy Code
//static std::std::wstring strConnection = _T("Driver={SQL Server};Server=(local);"
//        "Database=MyDatabaseName;Uid=MyUserName;Pwd=MyPassword;");
...
//
////If you want to connect to a local Database, you can use a connection std::wstring like the following:
////
//// Collapse | Copy Code
...
//Where test is the Name of MySQL Database. Also, you can replace the Name of the Database by the following connection std::wstring: server=localhost;DB=test.
//
